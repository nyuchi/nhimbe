...
Worker note: the `worker/` directory is now the **`nhimbe-mcp`** server — a task-based MCP at `events.mukoko.com/mcp` and the only thing that runs on Cloudflare Workers. It is **not** part of the app request path (feature work lives in `src/`); see "nhimbe MCP" below.
...
The `worker/` directory is now a single-purpose **task-based MCP server** (`nhimbe-mcp`) — the legacy REST backend, Supabase reads, Paynow, Resend email and queues were all stripped out. It is a stateless Streamable-HTTP MCP server (JSON-RPC 2.0, fetch-native, `hono`; no data of its own) — **dual-era**: legacy MCP revisions (2025-06-18/2025-03-26/2024-11-05, `initialize` handshake) plus **beta** support for the stateless 2026-07-28 revision (per-request `_meta`, `server/discover`, request-metadata header validation; built against the release candidate — see `worker/README.md` "Protocol support" and `worker/src/mcp/versions.ts`) — deployed on the mukoko zone at **`events.mukoko.com/mcp/*`** — the host is Cloudflare-proxied (orange cloud), and the Worker route intercepts `/mcp/*` on it.
...
- **nyuchi WorkOS org scoping** (`admin/src/lib/workos-org.ts`) — layered on top of the role gate: every requester must be an **active member of the nyuchi WorkOS organization**, else denied (→ `/denied`) **regardless of role** (org membership is *necessary*, not *sufficient*). The allowed org is resolved from **`WORKOS_ADMIN_ORG_ID`** when set (preferred — precise, no lookup), otherwise by the `nyuchi.com` domain via `organizations.listOrganizations` (cached per process). Membership is the `userManagement.listOrganizationMemberships` API (`statuses: ["active"]`, the source of truth; the AuthKit session `organizationId` is only a hint) and runs **before** the `identity.persons` lookup, so a non-member never hits the cluster. **Fail closed** — an unresolvable org or any WorkOS error denies. The `/denied` re-auth builds `getSignInUrl({ organizationId })` so the hosted sign-in screen is org-scoped (a hint; the server gate is the enforcement).
...
- `WORKOS_ADMIN_ORG_ID` *(admin app only; recommended in production)* — the nyuchi WorkOS **organization id** (`org_…`) allowed into the standalone admin app. Set on the **nhimbe-admin** Vercel project (prod + preview), not the public app. When set the org gate uses it directly; when unset the org is resolved by the `nyuchi.com` domain and cached per process. See `admin/README.md`.
