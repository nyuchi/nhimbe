...
- Consolidated Nhimbe into a single full-stack app deployed on Vercel — retired the separate REST backend; the `worker/` directory is now the task-based **`nhimbe-mcp`** server at `events.mukoko.com/mcp`.
